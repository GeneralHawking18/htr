...
class PARSeq(CrossEntropySystem):
    ...
    def training_step(self, batch, batch_idx) -> STEP_OUTPUT:
        ...
        loss /= loss_numel
        """if batch_idx % 5 == 0:
            print(self.sched.get_lr()[0])"""
            
        # self.log("loss", loss, prog_bar=True, logger=True, on_step=True)
        self.log("lr", round(self.sched.get_lr()[0], 5), prog_bar=True, logger=True, on_step=True)
        """self.log_dict(
            {
                'loss': loss, 
                "lr": self.sched.get_lr()[0],
            }, 
            prog_bar=True, logger=True, on_step=True, on_epoch=True, batch_size = self.batch_size
        )"""
        return loss
