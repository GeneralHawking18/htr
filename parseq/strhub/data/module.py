...
class SceneTextDataModule(pl.LightningDataModule):
    ...
    TEST_ALL = ("test_v6")# tuple(set(TEST_BENCHMARK_SUB + TEST_BENCHMARK + TEST_NEW))
    ...
    def train_dataset(self):
        if self._train_dataset is None:
            ...
            root = PurePath(self.root_dir, 'train_v6')
    ...
    def val_dataset(self):
        if self._val_dataset is None:
            transform = self.get_transform(self.img_size, self.augment)
            root = PurePath(self.root_dir, 'valid_v6')
            self._val_dataset = build_tree_dataset(root, self.charset_train, self.max_label_length,
                                                   self.min_image_dim, self.remove_whitespace, self.normalize_unicode,
                                                   transform=transform)
    ...
    def test_dataset(self):
        if self._test_dataset is None:
            ...
            root = PurePath(self.root_dir, 'test_v6')
            self._test_dataset = build_tree_dataset(root, self.charset_train, self.max_label_length,
                                                   self.min_image_dim, self.remove_whitespace, self.normalize_unicode,
                                                   transform=transform)
        return self._test_dataset

    def train_dataloader(self):
        train_and_val_dataset = ConcatDataset([self.train_dataset, self.val_dataset])
        return DataLoader(train_and_val_dataset, batch_size=self.batch_size, shuffle=True,
