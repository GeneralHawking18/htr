...
def main():
    ...
    kwargs.update({
        "charset_test": 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚÝàáâãèéêìíòóôõùúýĂăĐđĨĩŨũƠơƯưẠạẢảẤấẦầẨẩẪẫẬậẮắẰằẲẳẴẵẶặẸẹẺẻẼẽẾếỀềỂểỄễỆệỈỉỊịỌọỎỏỐốỒồỔổỖỗỘộỚớỜờỞởỠỡỢợỤụỦủỨứỪừỬửỮữỰựỲỳỴỵỶỷỸỹ-'
    })
    ...
    test_set = ["test_v6"]# sorted(set(test_set))
    print(len(datamodule.test_dataloaders(test_set)['test_v6'].dataset))
    results = {}
    ...
    for name, dataloader in datamodule.test_dataloaders(test_set).items():
        ...
        confidence = 0
        label_length = 0
        preds, targets, img_names = [], [], []
        for imgs, labels, names in tqdm(iter(dataloader), desc=f'{name:>{max_width}}'):
            res = model.test_step((imgs.to(model.device), labels, names), -1)['output']
            # print(res.result)
            total += res.num_samples
            correct += res.correct
            ned += res.ned
            # confidence +=res.confidence
            confidences.extend(res.confidence)
            label_length += res.label_length
            preds.extend(res.preds)
            targets.extend(res.labels)
            img_names.extend(names)
        # print(list(zip(preds, targets)))
    ...
    file_zip_path = f"parseq_v2_{cer}.zip"
