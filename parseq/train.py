...
from pytorch_lightning.callbacks import ModelCheckpoint, StochasticWeightAveraging, TQDMProgressBar, RichProgressBar
...
def main(config: DictConfig):
    ...
    checkpoint = ModelCheckpoint(monitor='cer', mode='min', save_top_k=3, save_last=True,
                                 filename='{epoch}-{step}-{cer:.4f}-{val_NED:.4f}')
    ...
    trainer: Trainer = hydra.utils.instantiate(config.trainer, logger=TensorBoardLogger(cwd, '', '.'),
                                               ...
                                               callbacks=[checkpoint, swa, CustomProgressBar(refresh_rate = 1, process_position=0)])
    ...
    dataset_metadata = {
      "title": "upload ckpt cer = 0.0446",
      "id": "generalhawking/parseq-ocr-ckpt",
      "licenses": [
        {
          "name": "CC0-1.0"
        }
    ...
    os.system(f"kaggle datasets version -p . -m 11_33_17_9_23")
"""
