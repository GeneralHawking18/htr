...
class TransformerOCRCTC:
    ...
    def train(self):
        ...
        while self.epoch < self.config.pl_params.pl_trainer.max_epochs:
            ...
            if val_info['neg_leven_dist'] > self.best_ckpt[1]:
                saved_ckpt = f"{self.config.pl_params.model_callbacks.dirpath}/best_ckpt_1.pth"
                    # self.config.pl_params.model_callbacks.filename.format()
                self.best_ckpt[0] = saved_ckpt
                self.best_ckpt[1] = val_info['neg_leven_dist']
                self.save_checkpoint(saved_ckpt)
            print("Current best: ", self.best_ckpt)
            last_saved_ckpt = f"{self.config.pl_params.model_callbacks.dirpath}/last.pth"
            self.save_checkpoint(last_saved_ckpt)
    ...
    def export_submission(self, save_dir = "."):
        ...
        submission_csv_file = f"{save_dir}/{model_name}_{self.epoch}.csv"# f"{model_name}_{best_cer_val}.zip"
        submission_dict = {
            "id": [],
            "answer": [],
            # "confidences": [],
